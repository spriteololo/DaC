...
function AddDemand() {
    ...
    if (nform >= 1) {
        ...
        for (i= 1; i <= nform; i++) {
            ...
            if (reg.test(breq) && document.getElementById("breq" + i)
                    .getElementsByTagName("a").length >= 4
            ) {
                if (!CheckPers(i)) {
                    var control_text = ""
                    +"<span style=background-color:yellowgreen;color:black;>ПРИНЯТЬ_ЗАЯВКУ_4</span>";
                    actIframe.document.getElementById("control_msg").innerHTML =
                        control_text + "<br>";
...
if(persIframe.soclanList.length>1) { // в лечебницу
    persIframe.setTimeout(
        "top.frames[1].frames['channel_3'].location.href=top.frames[1].med_room;",
        1500
    );
}