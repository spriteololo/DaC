...
function renameLater(){
    ...
    const cImg = findBy(top.frames['d_act'], "tag", "img", "title", "Введите символы")
    const canvas = top.frames['d_act'].document.createElement('canvas');
    canvas.id = "fakeCanvas"
    canvas.width = cImg.width;
    canvas.height = cImg.height;

    const parentElement = cImg.parentElement;
    parentElement.insertBefore(canvas, cImg);

    if (canvas) {
        const ctx = canvas.getContext('2d');
        ctx.drawImage(cImg, 0, 0, cImg.width, cImg.height);
        adjustContrast(canvas, 4)
        greyScale(canvas, 230)

        loadTesseract().then(() => {
            canvas.toBlob(recognizeTextFromBlob, 'image/jpeg');
        }).catch(error => {
            console.error('Ошибка загрузки Tesseract.js:', error);
        });

    } else {
        console.log("Контейнер не найден на странице.");
    }
}

function greyScale(canvas, threshold){
...
function adjustContrast(canvas, contrastFactor) {
    const ctx = canvas.getContext('2d');
...
function getRectangles(canvas) {
    ...
    for(let x = 0; x < canvas.width; x++) {
        ...
        for(let y = 0; y < canvas.height; y++) {
            const r = data[(y * canvas.width + x) * 4]; // Красный канал (в диапазоне 0-1)
            const g = data[(y * canvas.width + x) * 4 + 1]; // Зеленый канал (в диапазоне 0-1)
            const b = data[(y * canvas.width + x) * 4 + 2]; // Синий канал (в диапазоне 0-1)
    ...
    for(let i = 0; i < leftBounds.length; i++) {
        ...
        results.push({
            left: leftBounds[i] - 1,
            top: topBound - 1,
            width: rightBounds[i] - leftBounds[i] + 2,
            height: bottomBound - topBound + 2
        })
    }
...
async function recognizeTextFromBlob(blob, attempt = 0) {
    if(typeof Tesseract === 'undefined' ) {
        if(attempt == 3) {
            return
        } else {
            loadTesseract().then(() => recognizeTextFromBlob(attempt + 1))
        }
    } else {
        ...
        await worker.setParameters({
            oem: Tesseract.OEM.TESSERACT_LSTM_COMBINED,
            tessedit_pageseg_mode: Tesseract.PSM.SINGLE_CHAR,
            tessedit_char_whitelist: '0123456789abcdefghijklmnopqrstuvwxyz'
        });
        const values = [];
        let rectangles = getRectangles(top.frames['d_act'].document.getElementById("fakeCanvas"))
        for (let i = 0; i < rectangles.length; i++) {
            const { data }  = await worker.recognize(blob, { rectangle: rectangles[i] });
...
function loadTesseract() {
    return new Promise((resolve, reject) => {
        const script = top.frames['d_act'].document.createElement('script');
        script.src = 'https://cdn.jsdelivr.net/npm/tesseract.js@4/dist/tesseract.min.js';
