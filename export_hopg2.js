...
var LoadImg = "<img src=https://cdn.jsdelivr.net/gh/spriteololo/DaC/img/arrow/loading.gif width=16 height=16 border=0>"; //TODO
var element_limit_hp = ""
    + "<span style=\"margin-left:4px;padding-left:12px;"
    + "background:#D4D0C8 url(https://" + hostname_oil + "/img/arrow/ico_lifeup.png) no-repeat;" //TODO
...
function asAudio(name) {
    byid("as_audio").innerHTML = ""
        + "<audio autoplay>"
        + "<source src=\"https://" + hostname_oil + "/audio/" + name + "\" type=\"audio/mpeg\">" //TODO
        + "</audio>";
}
...
function ItemOperationCity(a) { // Bag
    if (a == 0) {
        ...
        byid("t").innerHTML = ""
            + "<a href=\"#\" onclick=\"AddJS(1,'_ItemOperationCity2.js');byid('t').innerHTML=LoadImg;\" " //TODO
            + "style=\"margin-left:50%;\">[Wear/Unwear]</a>";
    }
    if (a == 1) {
        top.frames["d_act"].location = "smith.html";
        byid("t").innerHTML = ""
            + "<a href=\"#\" onclick=\"AddJS(1,'_ioRepair.js');byid('t').innerHTML=LoadImg;\" " //TODO
            + "style=\"margin-left:60%;\">[RepairAll]</a>";
    }
    if (a == 2) {
        byid("t").innerHTML = ""
            + "<a href=\"#\" onclick=\"top.frames['d_act'].location='privsmith.html?unick='+d.nk+'';\" "
            + "style=\"margin-left:60%;\">[Kuznica]</a>";
    }
    if (a == 3) {
        byid("t").innerHTML = ""
            + "<a href=\"#\" onclick=\"AddJS(1,'_ioStorage.js');byid('t').innerHTML=LoadImg;\" " //TODO
    ...
    if (a == 4) {
        ...
        portal_parm[4] = "<img src=\"sawmill.html?actUser-GoToArenaFromHouse=" + MyHome + "&room=1024\" "
            ...
            + "byid('portal').innerHTML='Портал в Огранк?';\""
            + " width=1 height=16 border=0><span id=portal>Vhod v portal</span>";
...
function AddJS(n, xfile) {
    if (n == 0) {
        ...
        script.type = "text/javascript";
        script.src = "https://cdn.jsdelivr.net/gh/spriteololo/DaC/" + xfile + "?xdac=" + Math.random(); //TODO
...
function CreateDemand(map, minlvl, maxlvl, maxp) {
    ...
    var control_text = ""
        + "<span style=background-color:yellow;color:black;>СОЗД?ТЬ_З?ЯВКУ</span>"
...
function MainSwitch(t) {
    if (t == 0) {
        ...
        document.getElementById("rjob").innerHTML = ""
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox.gif width=48 height=22 onclick=MainSwitch(1);>"; //TODO
    }
    if (t == 1) {
        as_audio = ""
            + "<audio autoplay>"
            + "<source src=\"https://cdn.jsdelivr.net/gh/spriteololo/DaC/audio/achtung.mp3\" type=\"audio/mpeg\">"
            + "</audio>";
        document.getElementById("rjob").innerHTML = ""
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox-a.gif width=48 height=22 onclick=MainSwitch(0);>";//TODO
    }
    if (t == 2) {
        OnOffguard = 0;
        document.getElementById("rgua").innerHTML = ""
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox.gif width=48 height=22 onclick=MainSwitch(3);MainSwitch(4);>";//TODO
    }
    if (t == 3) {
        OnOffguard = 1;
        document.getElementById("rgua").innerHTML = ""
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox-a.gif width=48 height=22 onclick=MainSwitch(2);>"; //TODO
    }
    if (t == 4) {
        OnOffbuttons = 0;
        document.getElementById("rbat").innerHTML = ""
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox.gif width=48 height=22 onclick=MainSwitch(5);MainSwitch(2);>";//TODO
    }
    if (t == 5) {
        OnOffbuttons = 1;
        document.getElementById("rbat").innerHTML = ""
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox-a.gif width=48 height=22 onclick=MainSwitch(4);>";//TODO
    }
    if (t == 10) {
        val = byid("melt2").style.display;
        byid("melt2").style.display = (val ? "" : "none");
        byid("melt2down").innerHTML = (val ? "<img border=0 src=https://" + hostname_oil + "/img/arrow/melt2up.gif width=24 height=22 onclick=MainSwitch(10);>" : "<img border=0 src=https://" + hostname_oil + "/img/arrow/melt2down.gif width=24 height=22 onclick=MainSwitch(10);>"); //TODO
    }
    if (t == 11) { // mytime off
        OnOffMytime = 0;
        document.getElementById("mytime").innerHTML = ""
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox.gif width=48 height=22 " //TODO
    ...
    if (t == 12) { // mytime on
        ...
        MainSwitch(16);
        document.getElementById("mytime").innerHTML = ""
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox-a.gif width=48 height=22 " //TODO
...
function foundry(tm) {
    ...
    if (tm == 10) {
        document.getElementById("melt").innerHTML = ""
            ...
            + "background:url(https://" + hostname_oil + "/img/arrow/radar.gif) no-repeat right center;\" id=\"rjob\">"//TODO
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox.gif width=48 height=22 onclick=MainSwitch(1);>"//TODO
            + "</span>"
            + "</td>"
            + "<td>"
            + "<span style=\"display:block;width:48;height:22;"
            + "background:url(https://" + hostname_oil + "/img/arrow/fight.gif) no-repeat right center;\" id=\"rgua\">"//TODO
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox.gif width=48 height=22 onclick=MainSwitch(3);MainSwitch(4);>"//TODO
            + "</span>"
            + "</td>"
            + "<td>"
            + "<span style=\"display:block;width:48;height:22;"
            + "background:url(https://" + hostname_oil + "/img/arrow/battle.gif) no-repeat right center;\" id=\"rbat\">"//TODO
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox-a.gif width=48 height=22 onclick=MainSwitch(4);>"//TODO
            + "</span>"
            + "</td>"
            + "<td>"
            ...
            + "background:url(https://" + hostname_oil + "/img/arrow/paneling.gif) no-repeat right center;\" id=\"melt2down\">"//TODO
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/melt2down.gif width=24 height=22 onclick=MainSwitch(10);>"//TODO
            + "</span>"
            + "</td>"
            + "</tr>"
            ...
            + "background:url(https://" + hostname_oil + "/img/arrow/mytime.gif) no-repeat right center;\" id=\"mytime\">"//TODO
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox.gif width=48 height=22 "//TODO
            + "onclick=\"MainSwitch(12);\" oncontextmenu=\""
            + "val=byid('myfort').style.display;"
            + "if(val=='block') {"
            ...
            + "background:url(https://" + hostname_oil + "/img/arrow/myfort.gif) no-repeat right center;\" id=\"myfort\">" //TODO
            + "<img border=0 src=https://" + hostname_oil + "/img/arrow/checkbox.gif width=48 height=22 "//TODO
            + "onclick=\"MainSwitch(17);\" oncontextmenu=\""
            + "val=byid('mytime').style.display;"
            + "if(val=='block') {"
            ...
            + "background:url(https://" + hostname_oil + "/img/arrow/fight.gif) no-repeat;\" title=\"Лимит раундов\">"//TODO
            + "</div>"
            + "</td>"
            + "<td>"
            + "<div onclick=\"document.CrDemand.clonsum.click();\" "
            + "style=\"margin-top:2px;margin-left:0px;width:15px;height:12px;"
            + "background:url(https://" + hostname_oil + "/img/arrow/ico_unis.png) no-repeat;"//TODO
            ...
            + "background:url(https://" + hostname_oil + "/img/arrow/ico_lifeup.png) no-repeat;"//TODO
            ...
            + "background:url(https://" + hostname_oil + "/img/arrow/ico_move_to.png) no-repeat;"//TODO
            ...
            if (UNBS[ENEMY].sd != UNBS[ME.id].sd) {
                ...
                for (var i = 0; i < 6; i++) { // loop
                    ...
                    if (xobjstr0 == xobjstr1 && EnemyFind(0)) {
                        ...
                        byid("status").style.backgroundColor = "yellow";
                        byid("logc").innerHTML = "&#936; <b>УД?Р</b>";
...
function autotest() {
    if (!document.getElementById("control_panel")) { // control
        ...
        div.innerHTML = ""
            ...
            + "this.value='OFF';"
            + "}\" id=\"ctrl_btn\">"
            + "<span id=\"logb\">&#153;Слабо?мие и отвага</span>";
        div.innerHTML += "&nbsp;";
        ...
        if (amountenemy == 1) { // limit
            ...
            if (top.frames["d_pers"].document.CrDemand.act_castle.value == 1 && amountfriend == 1) {
                top.frames["d_pers"].document.CrDemand.act_castle.click();
                top.frames["d_pers"].document.getElementById("t").innerHTML = "НЕТ ?ГРОКОВ. СТОП.";
            }
    ...
    if (logLastRoundID >= top.frames["d_pers"].document.CrDemand.abround.value) { // limit-raund
        byid("logb").innerHTML = ""
            + "<span style=\"color:red;\">"
            + "&#215;Лимит " + top.frames["d_pers"].document.CrDemand.abround.value + " ра?нд"
            + "</span>";
        abAudio("call_message.mp3");
        return document.getElementById("ctrl_btn").value = "ON";
    } // end-limit-raund
    ...
    if (tl_sec < 55 && canmove1 == "visible" && UNBS[ME.id].hp < ME.mhp / ab_limit_hp && yes_mbCast == 1) { // hp-up
        if (top.frames["d_pers"].document.CrDemand.abHP.value == 1) {
            ...
            byid("status").innerHTML = "HP";
            byid("status").style.backgroundColor = "red";
            document.getElementById("logb").innerHTML = "<b>Во??тановить HP</b>";
            abAudio("call_in.mp3");
            UseMagCast(0, 0, mbHP);
            cn0 = 0; cn1 = 0;
            return tmID[0] = setTimeout("autotest()", 7000);
    ...
    if (tl_sec > 8 || tl_min > 0) { // STOCK
        if (cn0 == 0 && cn1 <= rxy && !at_stock) { // stock0
            ...
            byid("logb").innerHTML = "&#9674;Пои?к цели..";
    ...
    if (canmove1 == "visible" && amountEnemyTN >= amountenemy / 2 ||
        ...
        byid("logc").innerHTML = "&#937; <b>Переход " + (speed_autobat / 1000) + " ?ек.</b>";
        tmID[1] = setTimeout(""
            + "UseMagCast(777);"
            + "ab_hide=0;"
...
var addscript = function () {
    if (ME.id == 100214920 || ME.id == 203718846) { // bb sv59190 Primere hetzer //TODO
        var btn_name_0 = "МаГ?дар";
    ...
    if (ME.id == 202721325) { // Hopg?
        var btn_name_0 = "Разр??ить";
        var btn_name_1 = "Точный";
        var btn_name_2 = "Меткий";
        var btn_name_3 = "HP";
        var btn_name_4 = "Уверн?ть?я";
    ...
    //< !----------------------------------------------------->
    if (ME.id == 203168448) { // Божий од?ванчик
        var btn_name_0 = "Разр??ить";
        var btn_name_1 = "Нап?гать";
        var btn_name_2 = "Клич";
        var btn_name_3 = "HP";
        var btn_name_4 = "Вр.защита";
        var btn_name_5 = "СилаД?ха";
        var btn_name_6 = "Переман";
        var btn_cast_0 = "UseMagCast(1,0,mbDestroy,MyX,MyY);";
        var btn_cast_1 = "UseMagCast(1,1,mbFreeze);";
        var btn_cast_2 = "UseMagCast(0,1,mbCurse);";
    ...
    document.getElementById("buttons").innerHTML = ""
        ...
        + "<audio autoplay>"
        + "<source src=\"https://" + hostname_oil + "/audio/jungle4.mp3\" type=\"audio/mpeg\">" //TODO
        + "</audio>"
        ...
        + obj_hover_3 + ">Выкин?ть из боя</div>"
...
link[2] = "Test|javascript:void(0);|Легион Че?ти";
N = (document.all) ? 0 : 1;
var ob;
var over = false;
var pname;
...
var addact = function () {
    ...
    if (d.id == 202721325) { // Hopg?
        ...
        user_home = "<a href=\"#\" onclick=\"top.frames['d_act'].location='homeenter_hid_" + MyHome + ".html';\" "
            ...
            + "ItemOperationCity(3);\" style=\"font-size:8pt;margin-left:2px;\">[С?нд?к]</a>";
    }
    ...
    if (d.id == 203168448) { // Божий од?ванчик
        MyClan = 146;
        mbHP = 5018190;
    }
    ...
    var ddmbox = "<form name=CrDemand style=\"padding-right:0px;padding-left:0px;padding-bottom:0px;margin:0px;padding-top:0px\">"
        ...
        + "<tr>"
        + "<td>"
        + NewButton(0, "50px", "#8A492F", "#8A492F", "#FFEEC0", "gold", "Ко&#9827;е?о", ""
            + "top.frames['d_act'].location='fortunawheeldata_actUser-SpinWheel_1.html';", "", "")
        + NewButton(0, "55px", "#8A492F", "#8A492F", "#FFEEC0", "gold", "«С?мка»", "ItemOperationCity(0);", "", "")
        + (d.id == 9215 ?
            ...
            + "font-size:12px;font-family:Arial;text-align:center;float:left;cursor:hand;\">&#9824;&nbsp;К&nbsp;замк?&#9824;</div></a>"
            :
            "<a href=\"javascript:void(0);\" onclick=\"top.frames['d_act'].location='castle_room_1_cid_'+MyClan+'.html';\" "
            + "oncontextmenu=\"top.frames['d_act'].location='medroom_cid_'+MyClan+'.html';return false;\">"
            + "<div onmouseover=\"info(0,this,'gold')\" onmouseout=\"endi(this,'#FFEEC0')\" "
        ...
        + "font-size:12px;font-family:Arial;text-align:center;float:left;cursor:hand;\">К?зница&#189;</div></a>"
        ...
        + NewButton(0, "50px", "#8A492F", "#8A492F", "#FFEEC0", "gold", "«К&nbsp;маг?»", ""
        ...
        + NewButton(0, "55px", "#8A492F", "#8A492F", "#FFEEC0", "gold", "«Т?рнир»", "top.frames['d_act'].location='arenat.html';", "", "")
        ...
        + "text-align:center;float:left;cursor:hand;\">Оп??ка&nbsp;&#189;</div></a>"
        ...
        + "<option selected value=0>?танд.</option>"
        + "<option value=1>?ванпо?т</option>"
        + "<option value=2>Катакомбы</option>"
        + "<option value=3>За?ада</option>"
        + "<option value=4>Лабиринт</option>"
        + "<option value=5>Крепо?ть</option>"
        + "<option value=6>На за?тав?</option>"
        + "<option value=7>Переправа</option>"
        + "<option value=8>Вход в подземелье</option>"
        + "<option value=9>БЛ?Ц</option>"
        + "</select>"
...
var addObs = function () {
    ...
    // end-move-parm
    if (!LocSite("value", "input", "Прервать работ?") &&
        ...
        if (Moscow_hours == 10 && wheeluck_day != Moscow_day ||
            ...
            byid("t").innerHTML = "Коле?о ?дачи, День " + Moscow_day;
    ...
    if (demand == 3) { // active-demand
        ...
        if (y == 99 && document.CrDemand.act_castle.value == 1) { // move-demand
            ...
            if (!LocSite("name", "INPUT", "Battle{vall}") && nform != 0) { // в лечебниц? (от бандита)
                top.frames["d_act"].location = "arena_room_1_bmode_3.html";
                top.frames["d_pers"].setTimeout("top.frames['d_pers'].frames['channel_3'].location=med_room", 1500);
            }
            if (!LocSite("name", "INPUT", "Battle{vall}") && LocSite("title", "BUTTON", "Про?мотр повтора")) { // в лечебниц? (от бандита)
                top.frames["d_act"].location = "arena_room_1_bmode_3.html";
                top.frames["d_pers"].setTimeout("top.frames['d_pers'].frames['channel_3'].location=med_room", 1500);
            }
            if (!LocSite("name", "INPUT", "Battle{vall}") && LocSite("value", "INPUT", "При?е?ть")) { // в лечебниц? (от бандита)
                top.frames["d_act"].location = "arena_room_1_bmode_3.html";
                top.frames["d_pers"].setTimeout("top.frames['d_pers'].frames['channel_3'].location=med_room", 1500);
            }
        } // end-move-demand
    ...
    if (y == 99 && document.CrDemand.act_castle.value == 1 &&
        LocSite("value", "INPUT", "Подать заявк?")) { // wait-fight
        var e3 = document.all("dinjcell").innerHTML;
        var xhl = new RegExp(health, "g");
    ...
    if (document.CrDemand.act_castle.value == 1) { // hand-off
        ...
        if (rhandd == "к?лаки" || lhandd == "к?лаки") {
            document.CrDemand.act_castle.click();
            setTimeout("byid('t').innerHTML='Р?ЗОРУЖЕН';", 14000);
            byid("t").innerHTML = "Р?ЗОРУЖЕН"
                + "<audio autoplay loop>"
    ...
    if (OnOffMyfort == 1) { // На форпо?т напали!
        if (MyTime(1)) {
    ...
    if (y == 0) { // fight
        ...
        if (OnOffbuttons == 1) {
            if (buttons == 1) { // активировать кнопки
                ...
                AddJS(1, "export_hopg2.js");
            }
        }
        if (OnOffguard == 1) {
            if (guard == 1) { // активировать охран?
                guard = 0;
                guard_act = 1;
    ...
    if (LocSite("title", "BUTTON", "Про?мотр повтора") &&
        ...
        var e3 = document.all("dinjcell").innerHTML;
        var xhl = new RegExp(health, "g");
        if (xhl.test(e3)) { // в лечебниц?
            // msg-fun-log
            var script = top.frames["d_act"].document.createElement("script");
            script.type = "text/javascript";
            script.text = "function msgBadEvent() {"
                + "var bad_event=/о?тало?ь/.test(top.frames['d_pers'].document.getElementById('dinjcell').innerHTML);"
                + "if(bad_event) {"
...
function Run() {
    ...
    if (a > 0) { //TODO
        ...
        if (ME.id == 203718846) { // me
            mbClon = 681194;//TODO
            mbHP = 9905581;
            mbStone = 464506;//TODO
            mbFireBall = 657540;//TODO
            mbArmor = 681193;//TODO
            mbFreeze = 660931;//TODO
            mbPereman = 3558028;//TODO
            mbCurse = 3558029;//TODO
            abilityFireDust = 0;
            abilityKill = 0;
            abilityAbort = 0;
            abilityCloneDispel = 0;
