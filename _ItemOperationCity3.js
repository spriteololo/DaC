        ...
        if (Reg.test(txt)) {
            ...
            if (top.frames["d_pers"].d.nk == "RetRieveR") {
                top.frames["d_pers"].document.getElementById("t").innerHTML = ""
                    ...
                    + "onclick=\"ioTitle=2;AddJS(1,'_ItemOperationCity3.js');\" "
                    + "style=margin-left:15%;>[High]</a>"
                    + "<a href=\"javascript:byid('t').innerHTML=LoadImg;void(0);\" "
                    + "onclick=\"ioTitle=3;AddJS(1,'_ItemOperationCity3.js');\">[Medium]</a>"
                    + "<a href=\"javascript:byid('t').innerHTML=LoadImg;void(0);\" "
                    + "onclick=\"ioTitle=1;AddJS(1,'_ItemOperationCity3.js');\" "
                    + "oncontextmenu=\"ioTitle=4;AddJS(1,'_ItemOperationCity3.js');"
                    + "byid('t').innerHTML=LoadImg;return false;\">[Low&#189;]</a>";
                return document.location = "room.html";
            }
            top.frames["d_pers"].document.getElementById("t").innerHTML = ""
                + "<a href=\"javascript:byid('t').innerHTML=LoadImg;void(0);\" "
                + "onclick=\"top.frames['d_pers'].ioTitle=0;AddJS(1,'_ItemOperationCity3.js');\" "
    ...
    if (a == 1) { // одеть
        if (!Reg.test(txt)) {
            if (ioLocVar < top.frames["d_pers"].iTempData.length) {
                ...
                top.frames["d_pers"].document.getElementById("t").innerHTML = "<a href=\"#\" onclick=\"AddJS(1,'_ItemOperationCity3.js');\" style=margin-left:70%; title=" + a + ">[Раздеть]</a>";
    ...
    if (a == 2) { // Low
        if (!Reg.test(txt)) {
            if (ioLocVar < ioDressLow.length) {
                ...
                top.frames["d_pers"].document.getElementById("t").innerHTML = ""
                    + "<a href=\"#\" onclick=\"AddJS(1,'_ItemOperationCity3.js');\" style=margin-left:70%; title=" + a + ">[Раздеть]</a>";
                document.location = "room_mode_0_type_7.chtml";
            }
        }
    } // end-Low
