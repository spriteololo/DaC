...
function ItemOperationCity(a) { // Bag
    if (a == 0) {
        ...
        byid("t").innerHTML = ""
            + "<a href=\"#\" onclick=\"AddJS(1,'_ItemOperationCity3.js');byid('t').innerHTML=LoadImg;\" " 
            + "style=\"margin-left:50%;\">[Wear/Unwear]</a>";
    }
    if (a == 1) {
        top.frames["d_act"].location = "smith.html";
        byid("t").innerHTML = ""
            + "<a href=\"#\" onclick=\"AddJS(1,'_ioRepair2.js');byid('t').innerHTML=LoadImg;\" "
    ...
    if (a == 3) {
        byid("t").innerHTML = ""
            + "<a href=\"#\" onclick=\"AddJS(1,'_ioStorage2.js');byid('t').innerHTML=LoadImg;\" " 
...
function autobat(a) {
            ...
            if (UNBS[ENEMY].sd != UNBS[ME.id].sd) {
                ...
                for (var i = 0; i < 6; i++) { // loop
                    ...
                    if (xobjstr0 == xobjstr1 && EnemyFind(0)) {
                        ...
                        if(ME.id == 203718846) rnd = Math.round(Math.random()) + 4
                        switch (rnd) { // удар-блок
                            ...
                            case 3: SwitchAttack(1); ubkick(0, 0); ubkick(1, 0); MakeTurn();
                                ...
                                break; // голова-голова
                            case 4: SwitchAttack(1); ubblock(0, 1); ubblock(1, 2); ubblock(0, 3); ubblock(1, 4); MakeTurn();
                                byid("status").innerHTML = "k4";
                                break; // блок не голова
                            case 5: SwitchAttack(1); ubblock(0, 0); ubblock(1, 2); ubblock(0, 3); ubblock(1, 4); MakeTurn();
                                byid("status").innerHTML = "k5";
                                break; // блок не корпус
                        }
                    }
                } // end-loop
    ...
    if (a == 2) { // BLOCK
        if (cn0 == 0 && ab_move == 0) {
            ...
            if (ME.id == 203168448 || ME.id == 200411306 || ME.id == 203718846) { //block-style
...
function autotest() {
    if (!document.getElementById("control_panel")) { // control
        ...
        top.frames["d_chatact"].chclear();
        top.frames["d_chatact"].chclear();
        if (!DEAD[ME.id] && UNBS[ME.id].flg != 8) AddJS(1, "_jsgame_AddData2.js");
...
var addscript = function () {
    if (ME.id == 100214920 || ME.id == 203718846 || ME.id == 203704036 || ME.id == 201135707 || ME.id == 200411306) { // bb sv59190 Primere hetzer //TODO
    ...
    //< !----------------------------------------------------->
    if (ME.id == 200674992 || ME.id == 201053200) { // gar cem
    ...
    //< !----------------------------------------------------->
    if (ME.id == 203168448 || ME.id == 202132809) { // Божий од?ванчик drak
        var btn_name_0 = "Разрушить";
...
var addact = function () {
    ...
    if (d.id == 203718846) { // fant
        MyClan = 146;
        mbHP = 9905581;
    }

    if (d.id == 202132809) { // drak
        MyClan = 146;
        mbHP = 23756362;
    }
    ...
    if (d.id == 201053200) { // cem
        MyClan = 146;
        mbHP = 115626;
    }


    if (d.id == 203704036) { // plesh
        MyClan = 183;
        mbHP = 9324974;
    	MyHome = 648;
    	user_home = "<a href=\"#\" onclick=\"top.frames['d_act'].location='homeenter_hid_" + MyHome + ".html';\" "
    ...
    if (y == 0) { // fight
        ...
        if (OnOffbuttons == 1) {
            if (buttons == 1) { // активировать кнопки
                ...
                AddJS(1, "export_hopg8_6_no_chat.js");
            }
        }
        if (OnOffguard == 1) {
    ...
    if (a > 0) { //TODO
        ...
        if (ME.id == 200674992) { // garr
         ...
         if (ME.id == 201053200) { // cem
            ...
            mbSniper = 95432; //++
            mbJump = 88270; //++
            abilityFireDust = 640295;
            abilityKill = 0;
            abilityAbort = 476176;
            abilityCloneDispel = 681758;
            abilityPet = 0;
        }

        if (ME.id == 202132809) { // drak
            mbClon = 661462;//TODO +
            mbHP = 680124;
           	mbDestroy = 695105; //+++
           	mbFireBall = 660591;
