    ...
    for (var i in changes) {
        ...
        switch (+row.act) { // Собираем лог
            case 0: // Пустышка - зарезервирована
                break;
            case 1: // Переместился
                AddLogRecord(tm, UserOut(row.chng[0]) + ' походил');
                break;
            case 2: // Снял вещь;
            ...
            case 3: // Одел вещь;
                ...
                break;
            case 4: // Увернулся;
                AddLogRecord(tm, Ast(row.chng[1].unb) + UserOut(row.chng[1]) + ' <font class="uvor">' + 'увернулся' + '</font> от ' + UserOut(row.chng[0]) + ' ' + BodyParts[row.pt]);
                try {
            ...
            case 5: // Заблокировал;
                if (row.crt) {
                    dmgdsc = '<font class="krit">удар</font> ';
                } else {
                    dmgdsc = 'удар ';
                }
                if (row.rpl || row.rpl == 0) rpl = ' и <font class="otv">ответил</font> на ' + row.rpl;
                if (row.neprobiv) neprobiv = ' (перк)';
                AddLogRecord(tm, Ast(row.chng[1].unb) + UserOut(row.chng[1]) + ' заблокировал ' + dmgdsc + UserOut(row.chng[0]) + ' ' + BodyParts[row.pt] + rpl + neprobiv);
                try {
                ...
                break;
            case 6: // Удар прошел
                if (row.lck) dmgdsc = '<font class="luck">' + 'удачный' + '</font> ';
                if (row.crt) {
                    dmgdsc = dmgdsc + '<font class="krit">' + 'удар' + '</font> ';
                ...
                if (row.rpl || row.rpl == 0) rpl = ' но получил в <font class="otv">ответ</font> удар на <b>' + row.rpl + '</b>';
                AddLogRecord(tm, Ast(row.chng[1].unb) + ' ' + UserOut(row.chng[0]) + ' нанес ' + UserOut(row.chng[1]) + ' ' + dmgdsc + BodyParts[row.pt] + ' на ' + dmgn + tbl + rpl);
                try {
            ...
            case 24: // Удаление препятствия
                ...
                break;
            case 25: // Удар магией
                AddLogRecord(tm, Ast(row.chng[0].unb) + UserOut(row.chng[0]) + ' магом по ' + UserOut(row.chng[1]) + ' на ' + row.dmg);
                break;
            case 26: // Переман
                AddLogRecord(tm, Ast(row.chng[0].unb) + UserOut(row.chng[0]) + ' сманил ' + UserOut(row.chng[1]));
                break;
            case 27: // Антимагия
            ...
            case 34: // Урон магией стихий
                AddLogRecord(tm, Ast(row.chng[0].unb) + UserOut(row.chng[0]) + ' ударил мской ' + SMagicSchools[row.nmb] + ' ' + UserOut(row.chng[1]) + ' на <b>' + row.dmg + '</b>');
                break;
            ...
            case 65: // Поглотил магию Стихий
                AddLogRecord(tm, Ast(row.chng[1].unb) + UserOut(row.chng[1]) + ' поглотил магию стихий ' + UserOut(row.chng[0]));
                break;
