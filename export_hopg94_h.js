...
function findElem(elem, tag, txt) {
    const elems = top.frames["d_act"].document.getElementsByTagName(tag)
    if (elem == "title") {
        for (i = 0; i < elems.length; i++) {
            if (elems[i].title == txt) {
...
function parseDateAfter(txt) {
    const elem = findElem("innerText", "b", txt)
    if(elem && elem.nextSibling) {
        return parseDate(elem.nextSibling.textContent)
    }
}
...
var addObs = function () {
    ...
    if (LocSite("value", "input", "Прервать работу") ||
            LocSite("innerText", "b", "Вы устали. Осталось отдыхать:") ||
            LocSite("innerText", "b", "Вы готовы огранить камень. Повторите надпись на верстаке:") ||
            LocSite("innerText", "b", "Вы готовы приступить к работе. Повторите надпись на верстаке:") ||
            lastKnownWorkingTimeStamp) { // worktimer
        if (LocSite("value", "input", "Прервать работу")) {
            if(!lastKnownWorkingTimeStamp) {
                var ttl = parseDateAfter("Осталось от текущей работы:")
                const newTimeStamp = new Date(Date.now())
                if (ttl) {
                    newTimeStamp.setHours(newTimeStamp.getHours() + ttl.hours)
                    newTimeStamp.setMinutes(newTimeStamp.getMinutes() + ttl.minutes + Math.round(Math.random() * 5))
                    newTimeStamp.setSeconds(newTimeStamp.getSeconds() + ttl.seconds + Math.round(Math.random() * 59))
                } else {
                    console.log("couldn't get ttl")
                }
                lastKnownWorkingTimeStamp = newTimeStamp
            } else {
                const pathname = top.frames['d_act'].location.pathname
                if(lastKnownWorkingTimeStamp < new Date(Date.now()) &&
                    (pathname.includes("jewelry") || pathname.includes("smith"))) {
                        lastKnownWorkingTimeStamp = null;
                        const refr = findElem("title", "button", "Обновить")
                        if(refr) refr.click()
                    }
            }
        }
        if (LocSite("innerText", "b", "Вы устали. Осталось отдыхать:")) {
            if(!lastKnownWorkingTimeStamp) {
                var ttl = parseDateAfter("Вы устали. Осталось отдыхать:")
                const newTimeStamp = new Date(Date.now())
                if (ttl) {
                    newTimeStamp.setHours(newTimeStamp.getHours() + ttl.hours)
                    newTimeStamp.setMinutes(newTimeStamp.getMinutes() + ttl.minutes + Math.round(Math.random()))
                    newTimeStamp.setSeconds(newTimeStamp.getSeconds() + ttl.seconds + Math.round(Math.random() * 59))
                } else {
                    console.log("couldn't get ttl")
                }
        ...
        if (LocSite("innerText", "b", "Вы готовы огранить камень. Повторите надпись на верстаке:") ||
            LocSite("innerText", "b", "Вы готовы приступить к работе. Повторите надпись на верстаке:")) {
            lastKnownWorkingTimeStamp = null;
            asAudio("notif.mp3")
        }
